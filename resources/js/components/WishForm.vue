...
// server-side error key maps straight onto a field. A factory, so a successful
// submission can put the form back to where it started.
...
async function submit() {
	...
	try {
		...
		// Same reasoning as the error scroll: the confirmation sits above a form
		// long enough to leave it off screen.
		await nextTick();

		status.value?.scrollIntoView({
			behavior: 'smooth',
			block: 'center',
		});
	} catch {
...
<template>
	<section ref="root">
		...
		<template v-if="done">
			<div ref="status" role="status" class="text-brand text-[18px] md:text-[24px] lg:text-[28px]">
